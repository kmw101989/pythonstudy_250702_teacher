...
JOIN ranking B
ON B.item_code = A.item_code
WHERE main_category = "ALL"; # 만약 조건절에서 설정한 데이터값이 특정 테이틀에서만 사용하는 경우, 테이블 언급 생략
...
# 메인카테고리 ALL에서 "판매자별" 베스트상품 갯수를 출력해주세요!
...
# 메인 카테고리가 "패션의류"라는 전제하에
# 패션의류 전체 베스트상품에서 판매자별 베스트상품 갯수가 5이상인
# 판매자이름과 해당 판매자가 판매중인 베스트상품 갯수 출력!!!
